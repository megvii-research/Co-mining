...
class RetinaNet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.matching_iou_thres = cfg.MODEL.RETINANET.MATCHING_IOU_THRES
    ...
    def forward(self, batched_inputs):
        ...
        """
        images, images_aug = self.preprocess_image(batched_inputs)
        ...
        features_aug = [features_aug[f] for f in self.in_features]

        anchors = self.anchor_generator(features)
        anchors_aug = self.anchor_generator(features_aug)

        box_cls, box_delta, box_cls_aug, box_delta_aug = self.head(features, features_aug)
        ...
        if self.training:
            ...
            with torch.no_grad():
                ...
                head2_preds = self.pseudo_gt_generate(
                    [cr.clone() for cr in box_cls_aug],
                    [br.clone() for br in box_delta_aug],
                    anchors_aug,
                    images_aug
                )

                gt_instances1 = self.merge_ground_truth(gt_instances, head2_preds, images, self.matching_iou_thres)
                gt_instances2 = self.merge_ground_truth(gt_instances, head1_preds, images, self.matching_iou_thres)

                gt_classes1, gt_anchors_reg_deltas1 = self.get_ground_truth(anchors, gt_instances1)
                gt_classes2, gt_anchors_reg_deltas2 = self.get_ground_truth(anchors_aug, gt_instances2)

            head1_losses = self.losses_head1(gt_classes1, gt_anchors_reg_deltas1, box_cls, box_delta)
            head2_losses = self.losses_head2(gt_classes2, gt_anchors_reg_deltas2, box_cls_aug, box_delta_aug)

            return dict(head1_losses, **head2_losses)
        else:
    ...
    @torch.no_grad()
    def merge_ground_truth(self, targets, predictions, images, iou_thresold):
...
class RetinaNetHead(nn.Module):
    ...
    def forward(self, features, features_aug):
        """
        Arguments:
        ...
        logits2 = []
        bbox_reg2 = []
        for feature, features_aug in zip(features, features_aug):
            cls_feat = self.cls_subnet(feature)
            cls_feat_aug = self.cls_subnet(features_aug)
            logits1.append(self.cls_score(cls_feat))
            logits2.append(self.cls_score(cls_feat_aug))

            reg_feat = self.bbox_subnet(feature)
            reg_feat_aug = self.bbox_subnet(features_aug)
            bbox_reg1.append(self.bbox_pred(reg_feat))
            bbox_reg2.append(self.bbox_pred(reg_feat_aug))

        return logits1, bbox_reg1, logits2, bbox_reg2
