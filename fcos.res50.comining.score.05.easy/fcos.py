...
class FCOS(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        shifts_aug = self.shift_generator(features_aug)

        cls, delta, center, cls_aug, delta_aug, center_aug = self.head(features, features_aug)
        ...
        if self.training:
            with torch.no_grad():
                ...
                gt_classes_fcos1, gt_shifts_reg_deltas1, gt_centerness1 = self.get_fcos_ground_truth(
                    shifts, gt_instances1)

                gt_classes_fcos2, gt_shifts_reg_deltas2, gt_centerness2 = self.get_fcos_ground_truth(
                    shifts_aug, gt_instances2)
            ...
            fcos_losses2 = self.fcos_losses2(gt_classes_fcos2, gt_shifts_reg_deltas2, gt_centerness2,
                                           cls_aug, delta_aug, center_aug)

            return dict(fcos_losses1, **fcos_losses2)

        else:
            results = self.inference(cls, delta, center, shifts, images)
    ...
    def pseudo_gt_generate(self, box_cls, box_delta, box_center, shifts, images):
        """
        Arguments:
            box_cls, box_delta, box_center: Same as the output of :meth:`FCOSHead.forward`
            shifts (list[list[Tensor]): a list of #images elements. Each is a
...
class FCOSHead(nn.Module):
    ...
    def forward(self, features, features_color):
        """
        Arguments:
